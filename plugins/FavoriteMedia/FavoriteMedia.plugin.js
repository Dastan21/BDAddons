				...
				render() {
					return React.createElement("div", {
						...
						this.props.type === "audio" ? React.createElement("div", {
							...
							React.createElement("span", { className: classes.category.categoryName }, this.props.name + this.props.ext)
						) : null
					)
				}
			}
			...
			return class FavoriteMedia extends Plugin {
				...
				patchChannelTextArea() {
					Patcher.after(ChannelTextArea, "render", (_, [props], returnValue) => {
						...
						if (!ChannelStore.getChannel(SelectedChannelStore.getChannelId()).type && !getPermissionsArray(Permissions.getChannelPermissions(ChannelStore.getChannel(SelectedChannelStore.getChannelId())).toString()).includes(Number(PermissionsConstants.SEND_MESSAGES.toString()))) return;
