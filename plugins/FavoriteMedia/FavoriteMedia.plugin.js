...
class FavoriteMedia {
	...
	classes = {
		...
		embedFull: BdApi.findModuleByProps("spoilerBlurRadius", "embed", "embedFull").embedFull,
	};
	...
	checkForMedias(node) {
		for (let media of node.childNodes) {
			if (media.classList.contains(this.classes.embedFull.split(' ')[0])) return;
			const img = media.querySelector("a > img"); if (img) this.addFavButtonOnImage(img.parentNode);
